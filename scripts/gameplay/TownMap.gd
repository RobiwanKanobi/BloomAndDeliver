...
func _setup_locations() -> void:
	for loc_node in _locations_node.get_children():
		...
		if not GameState.is_location_unlocked(location_id):
			loc_node.visible = false
			continue

		loc_node.visible = true
...
func _complete_delivery() -> void:
	...
	if order:
		var reward = RewardSystem.calculate_delivery_reward(order, GameState.current_bouquet_flower_ids)
		GameState.money += reward.money
		GameState.reputation += reward.reputation
		GameState.money_changed.emit()

		RelationshipSystem.add_points(order.customer_id, 1)
		JournalSystem.record_delivery(order.customer_id, order.id)
		GameState.complete_order(order.id)

		var bonus_text := ""
		if reward.preference_bonus > 0:
			bonus_text += " (+%d preference)" % reward.preference_bonus
		if reward.variety_bonus > 0:
			bonus_text += " (+%d variety)" % reward.variety_bonus
		_status_label.text = "Delivery complete! +%d coins%s" % [reward.money, bonus_text]

		EventBus.order_completed.emit(order.id, order.customer_id)
		EventBus.delivery_completed.emit(GameState.current_delivery_target_id, reward.money)
...
func _update_destination_display() -> void:
	if GameState.current_delivery_target_id != "":
		var loc = ContentDB.get_location(GameState.current_delivery_target_id)
